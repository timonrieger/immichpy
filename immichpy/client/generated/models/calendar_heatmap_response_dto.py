...
class CalendarHeatmapResponseDto(BaseModel):
    ...
    to: StrictStr = Field(
        description="End date in UTC", json_schema_extra={"examples": ["2024-12-31"]}
    )
    total_count: Annotated[int, Field(le=9007199254740991, strict=True, ge=0)] = Field(
        description="Total activity count over the period", alias="totalCount"
    )
        ...
        _obj = cls.model_validate(
            {
                "from": obj.get("from"),
                "series": [
                    CalendarHeatmapResponseDtoSeriesInner.from_dict(_item)
                    for _item in obj["series"]
                ]
