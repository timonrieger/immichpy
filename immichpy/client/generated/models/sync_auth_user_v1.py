...
class SyncAuthUserV1(BaseModel):
    ...
    profile_changed_at: datetime = Field(
        ...
        alias="profileChangedAt",
        json_schema_extra={"examples": ["2024-01-01T00:00:00Z"]},
    )
    quota_size_in_bytes: Optional[
        Annotated[int, Field(le=9007199254740991, strict=True, ge=-9007199254740991)]
    ] = Field(description="Quota size in bytes", alias="quotaSizeInBytes")
    quota_usage_in_bytes: Annotated[
        int, Field(le=9007199254740991, strict=True, ge=-9007199254740991)
    ] = Field(description="Quota usage in bytes", alias="quotaUsageInBytes")
