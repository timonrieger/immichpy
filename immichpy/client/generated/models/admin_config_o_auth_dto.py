...
class AdminConfigOAuthDto(BaseModel):
    ...
    account_management_url: Optional[StrictStr] = Field(
        default="", description="Account management URL", alias="accountManagementUrl"
    )
    allow_insecure_requests: StrictBool = Field(
        description="Allow insecure requests", alias="allowInsecureRequests"
    )
    ...
    default_storage_quota: Optional[
        Annotated[int, Field(le=9007199254740991, strict=True, ge=0)]
    ] = Field(description="Default storage quota", alias="defaultStorageQuota")
    enabled: StrictBool = Field(description="Enabled")
    end_session_endpoint: StrictStr = Field(
        description="End session endpoint", alias="endSessionEndpoint"
    )
    ...
    signing_algorithm: StrictStr = Field(
        description="Signing algorithm", alias="signingAlgorithm"
    )
    ...
    timeout: Annotated[int, Field(le=9007199254740991, strict=True, ge=1)] = Field(
        description="Timeout"
    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "accountManagementUrl": obj.get("accountManagementUrl")
                if obj.get("accountManagementUrl") is not None
                else "",
