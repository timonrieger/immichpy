...
immichpy isn't a hand-written abstraction. It's autogenerated directly from the Immich OpenAPI schema using the well-established [openapi-generator](https://github.com/OpenAPITools/openapi-generator). Its core is mostly models and endpoint mappings rather than complex custom logic. If needed, the project can be easily forked or regenerated from the schema. On top of this core, a small set of hand-written [convenience features](./features.md) are provided.

### 2. Focused on API stability
The primary goal of immichpy is to track Immich API changes quickly and maintain compatibility with server releases. It acts as a stable buffer layer so downstream projects don't have to adjust to every API change themselves. Read more about [versioning](/overview/versioning).
